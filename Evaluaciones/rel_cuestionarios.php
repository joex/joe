...
$sql="SELECT nombre,tema,ideval from evaluaciones WHERE estado='1'";
$psql=mysql_query($sql)or die(mysql_error());
$importar="";
$tema="";
while($fetch=mysql_fetch_assoc($psql))
{$importar .='<tr><td>'.$fetch["nombre"].'</td><td>'.$fetch["tema"].'</td><td><input type="button" class="editar" value="EDITAR"><input type="button" class="desactivar" value="DESACTIVAR"><input type="hidden" name="oc[]" value="'.$fetch["ideval"].'"></td></tr>';
}
